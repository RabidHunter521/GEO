...
> - **2.1** FK/hot-column indexes — migration `f1a2b3c4d5e6_add_performance_indexes`. **Applied to the DB** (alembic now at head `f1a2b3c4d5e6`; the pending `e4f5a6b7c8d9` logo_url migration was applied at the same time). Correction: the newer feature tables (action_recommendations, content_*, ai_traffic_snapshots) and `scan_query_results(scan_id)` were *already* indexed — the gap was on the older core tables (scans, geo_scores, competitors, activity_log, reports), which this migration covers.
...
| # | Finding | Severity | Location |
|---|---------|----------|----------|
| 2.1 | **Missing indexes on the older core tables' FK / hot filter columns.** ✅ Fixed (migration `f1a2b3c4d5e6`, applied). The newer feature tables already had `client_id` indexes and `scan_query_results(scan_id)` was already covered, but `scans`, `geo_scores`, `competitors`, `activity_log`, and `reports` filtered `client_id`/ordered by `*_at` with no index. As scan data grows (~112 `scan_query_results` rows per scan) those degraded to sequential scans. Added composite indexes — see SQL block below. | **High** | core `models/*.py` |
